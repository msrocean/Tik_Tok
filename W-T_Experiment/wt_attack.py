...
def attack(X_train, y_train, X_test, y_test, unmon_label, args, VERBOSE=1):
    ...
    y_va = np_utils.to_categorical(y_test, classes)
...
def main():
    ...
    for mon_dir, unmon_dir in dirs:
        Xm, ym, class_map = load_data(os.path.join(args.traces, mon_dir), typ=args.attack, unmon=False, class_map=class_map)
        Xu, _, _ = load_data(os.path.join(args.traces, unmon_dir), typ=args.attack, unmon=True)
